...
public:
	...
	{
		while (AniadirMisionesCompletadas() < 2)
		{
			...
			if (vidas <= 0 && !Ganaste)
			{
				Ganaste = true;
				Console::Clear();
				pantalla->DibujarPantallaPerdio(); 
			}
			...
			if (!enSpawn)
			{
				if (enEscenario1)
				{
					if (escenario1->getTiempo() <= 0 && !QuiteVida) 
					{
						QuitarVidas();
						QuiteVida = true;
					}
				...
				{
					if (escenario2->getTiempo() <= 0 && !QuiteVida)   
					{
						QuitarVidas(); 
						QuiteVida = true; 
					}
				...
				{
					if (escenario3->getTiempo() <= 0 && !QuiteVida)   
					{
						QuitarVidas(); 
						QuiteVida = true; 
					}
	...
	{
		MundosCompletados = 0;
		if (GetMundo1Completado()) MundosCompletados++;
		if (GetMundo2Completado()) MundosCompletados++;  
		if (GetMundo3Completado()) MundosCompletados++; 
